...
public:
	...
	Model(const std::string& path, std::vector<glm::mat4> modelPositions, std::vector<glm::mat3> modelNormalMat);
	void DrawModel();
	void RecomputeModel(std::vector<glm::mat4>& modelPositions, std::vector<glm::mat3>& modelNormalMat);
private:
	void LoadModel(const std::string& path);
	void ProcessNode(aiNode* node, const aiScene* scene);
	Mesh ProcessMesh(aiMesh* mesh);
	std::vector<glm::mat4> modelPositions;
	std::vector<glm::mat3> modelNormalMat;
};
